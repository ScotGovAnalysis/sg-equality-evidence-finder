...
timeSeriesUI0 <- function(id,graphOptions=list()) {
  ...
  tagList(
    div(class="eef-graph-header",
      ...
      fluidRow(
        column(12,id=nsChart("displayPlot"),class="eef-plot",dygraphOutput(nsChart("plot"), height = 400)%>%withSpinner())#,
...
timeSeriesUI1 <- function(id,graphOptions=list()) {
  ...
  tagList(
    div(class="eef-graph-header",
      ...
      fluidRow(
        column(9,id=nsChart("displayPlot"),class="eef-plot ",dygraphOutput(nsChart("plot"), height = 400)%>%withSpinner()),
        ...
        column(9,id=nsChart("displayPlot"),class="eef-plot ",
               tabsetPanel(tabPanel("Chart",br(),
                           ...
                           tabPanel("Table",br(),
                                    ...
                                    uiOutput(ns("title2")),
                                    dataTableOutput(nsChart("Dashboardtable"))%>%withSpinner()
                           )
               ),
               uiOutput(ns("source")),
               p("Visit the",
...
lineChartUI0 <- function(id,graphOptions=list()) {
  ...
  tagList(
    div(class="eef-graph-header",
      ...
      fluidRow(
        column(12,id=nsChart("displayPlot"),class="eef-plot ",uiOutput(nsChart("plot"), height = 400))#%>%withSpinner())
      )#,
...
pieChartUI1 <- function(id,graphOptions=list()) {
  ...
  tagList(
    div(class="eef-graph-header",
      ...
      fluidRow(
        column(9,id=nsChart("displayPlot"),class="eef-plot",uiOutput(nsChart("plot"), height = 400)%>%withSpinner()),
        column(3,id=nsChart("displayOptions"),
               #tags$button(id = nsChart("toggledash"), "Show/Hide", href = "#", class="eef-options-toggle greyscalebutton"),
