...
#          seriesColour=NA)
NPFdata <- mutate(NPFdata,Date=as.Date(Date),
                  #Year=as.numeric(Year),
                  Figure=as.numeric(Figure),
                  Measure=Indicator) %>% #Measure is used for switching between multiple charts in e.g. timeSeries2 - we want to be switching between indicators usually for NPF data
  mutate(#yearOnly=is.na(Date),
         #intervalType=ifelse(EEFformat%in%c("",NA),ifelse(is.na(Date),"Year","Month"),EEFformat),
...
#apply corrects to NPFdataset (add "All Children" to age and gender disaggregations as not being flagged as totals in the NPF datasets)
NPFdata <- NPFdata %>%
...
NPFindex <- NPFdata %>% group_by(Outcome,Indicator,Disaggregation) %>% summarise(graphTitle=graphTitle[1],Source=Source[1]) %>% ungroup
...
print(paste(length(unique(EEFindex$topicID[!is.na(EEFindex$topicID)])),"topic sections loaded"))
