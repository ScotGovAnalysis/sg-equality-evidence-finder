...
asn <- round(as.numeric(filter(School_Education, Measure == "% school leavers in a positive follow-up destination",
                            Characteristic == "Disability",  Breakdown == "ASN", Year == max(Year))$Figure),1)

noasn <- round(as.numeric(filter(School_Education, Measure == "% school leavers in a positive follow-up destination",
...
* School leavers with additional support needs in `r latestyear`, continue to be less likely to enter positive destinations on leaving school, with `r asn` 
	per cent in positive follow-up destinations compared to `r noasn` per cent for those with no additional support needs.
