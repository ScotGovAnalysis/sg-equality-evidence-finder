rmdServer <- function(input,output,session,filterGraph,graphOptions) {
  if(!is.null(graphOptions$updateRmd)) {
    output$markdown <- renderUI({
      ...
      knit(text=readLines(panelData$inputMarkdownFile,warn=FALSE), quiet = TRUE,encoding="UTF-8") %>%
...
dygraphServer <- function(input,output,session,filteredGraph=reactive({NULL}),graphOptions=list(),maxRange=reactive({c(NA,NA)})) {
  ns <- session$ns
  panelData <- graphOptions$panelData
  ...
  output$plot <- renderDygraph({
    ...
    if(nrow(annotations)>0) for(r in 1:nrow(annotations)) plot <- dyAnnotation(plot,annotations$Date[r],annotations$annotationText[r],tooltip=annotations$annotationText[r],series=annotations$Breakdown[r],width=16+7*nchar(annotations$annotationText[r]),height=20,attachAtBottom = F)
    plot
  })
...
pieChartServer <- function(input,output,session,filterGraph=reactive({NULL}),graphOptions=list()) {
  ns <- session$ns
...
barChartServer <- function(input,output,session,filterGraph=reactive({NULL}),graphOptions=list(),maxRange=reactive({c(NA,NA)})) {
  ns <- session$ns
...
timeSeriesServer0 <- function(input,output,session,filterData=reactive({NULL}),graphOptions=list()) {
  ns <- session$ns
...
timeSeriesServer1 <- function(input,output,session,filterData=reactive({NULL}),graphOptions=list()) {
  ns <- session$ns
...
timeSeriesServer2 <- function(input,output,session,filterData=reactive({NULL}),graphOptions=list()) {
  ns <- session$ns
  #help
  shinyjs::onclick("info",{
...
npfServer3 <- function(input,output,session,filterData=reactive({NULL}),graphOptions=list(),filterChar=reactive({NULL})) {
  ...
  filterOptions3 <- reactive({
    ...
    filterTotal <- filter(filterData(),Characteristic %in% "Total",Disaggregation %in% "Total")
...
pieChartServer0 <- function(input,output,session,filterData=reactive({NULL}),graphOptions=list()) {
  ns <- session$ns
  ...
  callModule(downloadServer,"chart",filterGraph)
  if(!is.null(graphOptions$updateRmd)) callModule(rmdServer,"chart",filterGraph,graphOptions)
  callModule(pieChartServer,"chart",filterGraph,graphOptions)
  
}

pieChartServer1 <- function(input,output,session,filterData=reactive({NULL}),graphOptions=list()) {
  ns <- session$ns
...
pieChartServer2 <- function(input,output,session,filterData=reactive({NULL}),graphOptions=list()) {
  ns <- session$ns
...
barChartServer0 <- function(input,output,session,filterData=reactive({NULL}),graphOptions=list()) {
  ns <- session$ns
  ...
  callModule(downloadServer,"chart",filterGraph)
  if(!is.null(graphOptions$updateRmd)) callModule(rmdServer,"chart",filterGraph,graphOptions)
  callModule(barChartServer,"chart",filterGraph,graphOptions,maxRange)
  
}

barChartServer1 <- function(input,output,session,filterData=reactive({NULL}),graphOptions=list()) {
  ns <- session$ns
...
barChartServer2 <- function(input,output,session,filterData=reactive({NULL}),graphOptions=list()) {
  ns <- session$ns
...
barChartServer3 <- function(input,output,session,filterData=reactive({NULL}),graphOptions=list()) {
  ns <- session$ns
