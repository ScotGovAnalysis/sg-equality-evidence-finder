##modified dygraphs-extra script##
#added google charts download functions


gvisRegister <- function(gvis,id) {
  gvis$html$chart["jsDrawChart"] <- sub("chart\\.draw\\(data,options\\);",
                                        #paste0("google.visualization.events.addListener(chart, 'ready', function () {alert($('#empSLL-14-plot').width() );Shiny.onInputChange('",URIid,"',chart.getImageURI());});\nchart.draw(data,options);"),
                                        paste0("google.visualization.events.addListener(chart, 'ready', function () {if($('#",gvis$chartid,"').height()>0){GoogleExport.Vars['",id,"'] = chart;}});\nchart.draw(data,options);"),
                                        #paste0("google.visualization.events.addListener(chart, 'ready', function () {if($('[data-value=#",id,"').css('display')!='none'){GoogleExport.Vars['",id,"'] = chart;}});\nchart.draw(data,options);"),
                                        gvis$html$chart["jsDrawChart"])
  invisible(gvis)
}
...
gvisDownload =
  ...
  function(id, outputId, label, elid = NULL, usetitle = TRUE, asbutton = FALSE, class= "btn-default"){
    enc = function(x){
    ...
    if(is.null(elid)) elid = paste0(id[1], "-gvisDownload")
    out = tags$a(id = elid, class = "download-link",target="_blank",
                 href = "#", label, onclick =
                   paste0("GoogleExport.DownloadByID(",
                          encid,"); return false;"))
    if(asbutton){
      out$attribs$class = paste("btn", class, "download-link")
      out$children = tagList(icon("download"), out$children)
    }
    tagList(out,downloadLink(outputId," ",`data-plot`=id,style="display:none"))
  }
...
dyDownload =
  ...
  function(id, outputId, label, elid = NULL, usetitle = TRUE, asbutton = FALSE, class= "btn-default"){
    ...
    out = tags$a(id = elid, class = "download-link",target="_blank",
                 href = "#", label, onclick =
                   paste0("DygraphExport.DownloadByID(",
                          encid, ", ", enc(usetitle), "); return false;"))
    if(asbutton){
      out$attribs$class = paste("btn", class, "download-link")
      out$children = tagList(icon("download"), out$children)
    }
    tagList(out,downloadLink(outputId,"NULL",`data-plot`=id,style="display:none"))
  }
