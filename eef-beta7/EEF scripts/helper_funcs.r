  ...
  if(is.null(graphOptions$query)) {
    ...
    graphData <- graphData %>% 
      mutate(intervalType=NA,
             DateString=as.Date(Date)-1,#sparql query template uses interval end date as the Date (sparql end date is first day AFTER the interval ends so need to subtract 1 day)
...
jsFormatCommas <- function(d) paste0("
...
pubSource <- function(x,extra_details="",notes="",max_date=Sys.Date()) {
  ...
  match_source <- filter(EEFsources,series_name %in% x,sort_date<=max_date) %>%
    group_by(series_name) %>%
    arrange(desc(published_date),desc(source_updated)) %>%
    filter(row_number()==1) %>%
    ungroup
  ...
  if(nrow(match_internal)>0) {
    ...
    if(!notes%in%c(NA,"")) {
      ...
      notes <- strsplit(notes,"\n")[[1]] #treat new lines in excel cell as seperate (numbered) notes
