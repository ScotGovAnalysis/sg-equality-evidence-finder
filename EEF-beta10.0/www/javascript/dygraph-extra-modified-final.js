/*
Modified to:
- add google charts register and download functions
- renamed dygraph functions as was loading before dygraphs.js and functions were getting overwritten*/
...
// Convenience functions for downloading google charts
// Allow registration of google charts, which are stored in Vars
GoogleExport ={};
GoogleExport.Vars = {};
GoogleExport.Register = function(chart,id){
  GoogleExport.Vars[id] = chart;
};

GoogleExport.DownloadByID = function(id){
  var chart = GoogleExport.Vars[id];
  var datauri = chart.getImageURI();
  Shiny.onInputChange(id+'URI',datauri.replace("data:image/png;base64,", ""));
  setTimeout(function(){$('[data-plot=\"'+id+'\"]')[0].click();},500);
  return;
};

DygraphExport = {};

DygraphExport.DEFAULT_ATTRS = {
...
DygraphExport.ComputeAttrs = function(dygraph, canvas){
  var opts = {};
 ...
 */
DygraphExport.isSupported = function () {
    "use strict";
    try {
 ...
 *
 */
DygraphExport.asPNG = function (dygraph, img, userOptions) {
    "use strict";
    var canvas = DygraphExport.asCanvas(dygraph, userOptions);
 ...
 *
 */
DygraphExport.asCanvas = function (dygraph, userOptions) {
    "use strict";
    var options = {};
    var canvas = Dygraph.createCanvas();
    canvas.width = dygraph.width_;
    
    Dygraph.update(options, DygraphExport.DEFAULT_ATTRS);
    // Override default options with computed options
    Dygraph.update(options, DygraphExport.ComputeAttrs(dygraph, canvas));
    ...
    DygraphExport.drawPlot(canvas, dygraph, options);    
    DygraphExport.drawLegend(canvas, dygraph, options);
 ...
 */
DygraphExport.drawPlot = function (canvas, dygraph, options) {
    "use strict";
    var ctx = canvas.getContext("2d");
    ...
    var axesPluginDict = DygraphExport.getPlugin(dygraph, 'Axes Plugin');
    if (axesPluginDict) {
        var axesPlugin = axesPluginDict.plugin;
        
        for (i = 0; i < axesPlugin.ylabels_.length; i++) {
            DygraphExport.putLabel(ctx, axesPlugin.ylabels_[i], options,
                options.labelFont, options.labelFontColor);
        }
        
        for (i = 0; i < axesPlugin.xlabels_.length; i++) {
            DygraphExport.putLabel(ctx, axesPlugin.xlabels_[i], options,
                options.labelFont, options.labelFontColor);
        }
    }
    ...
    var labelsPluginDict = DygraphExport.getPlugin(dygraph, 'ChartLabels Plugin');
    if (labelsPluginDict) {
        var labelsPlugin = labelsPluginDict.plugin;

        DygraphExport.putLabel(ctx, labelsPlugin.title_div_, options, 
            options.titleFont, options.titleFontColor);

        DygraphExport.putLabel(ctx, labelsPlugin.xlabel_div_, options, 
            options.axisLabelFont, options.axisLabelFontColor);

        DygraphExport.putVerticalLabelY1(ctx, labelsPlugin.ylabel_div_, options, 
            options.axisLabelFont, options.axisLabelFontColor, "center");

        DygraphExport.putVerticalLabelY2(ctx, labelsPlugin.y2label_div_, options, 
            options.axisLabelFont, options.axisLabelFontColor, "center");
    }

    // Annotations
    DygraphExport.drawAnnotes(dygraph, ctx, options);
};
 ...
 */
DygraphExport.putLabel = function (ctx, divLabel, options, font, color) {
    "use strict";
    ...
    DygraphExport.putText(ctx, left, top, divLabel, font, color);
};
 
/**
 * Draws a label Y1 rotated 90 degrees counterclockwise.
 */
DygraphExport.putVerticalLabelY1 = function (ctx, divLabel, options, font, color, textAlign) {
    "use strict";
    if (!divLabel) {
        return;
    }
 ...
 */
DygraphExport.putVerticalLabelY2 = function (ctx, divLabel, options, font, color, textAlign) {
    "use strict";
    if (!divLabel) {
        return;
    }
 ...
 */
DygraphExport.putText = function (ctx, left, top, divLabel, font, color) {
    "use strict";
 ...
 *
 */
DygraphExport.drawLegend = function (canvas, dygraph, options) {
    "use strict";
    var ctx = canvas.getContext("2d");
...
DygraphExport.drawAnnotes = function(dygraph, ctx, options){
  ...
  for(i = 0; i < annotedivs.length; i++){
    ...
    DygraphExport.drawBox(ctx, pos.left, pos.top, width, height,
...
DygraphExport.drawBox = function(ctx, x, y, width, height, stroke, fill){
  ctx.beginPath();
 ...
 */
DygraphExport.getPlugin = function(dygraph, name) {
...
DygraphExport.Vars = {};
DygraphExport.VarsNames = function(){
  var names = new Array;
  for(var curname in DygraphExport.Vars){names.push(curname);}
  return names;
};
DygraphExport.Register = function(dygraph){
  var id = dygraph.maindiv_.id;
  DygraphExport.Vars[id] = dygraph;
};

// Allow downloading of registered dygraphs as a PNG
DygraphExport.DownloadByID = function(id, usetitle){
  ...
  var getPNG = function(dygraph){
    ...
    return {
      datauri: DygraphExport.asCanvas(dygraph).toDataURL(),
      filename: name + ".png"
    };
  };
  ...
  if(!isArray(id)){
    ...
    var curdygraph = DygraphExport.Vars[id];
    ...
    for(var i = 0; i < id.length; i++){
      var curdygraph = DygraphExport.Vars[id[i]];
      if(curdygraph != undefined){
        var curPNG = getPNG(curdygraph);
    ...
    var title = usetitle === true ? DygraphExport.Vars[id[0]].user_attrs_.title : id[0];
...
DygraphExport.ShinyTabResize = function(){
   $(document).on("click", 'a[data-toggle="tab"]', function(){
      var dyvars = DygraphExport.Vars;
